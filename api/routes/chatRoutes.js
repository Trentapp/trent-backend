...
chatRouter.post("/sendMessage", async (req, res) => {
	try {
		...
		if (!req.body.uid || !(req.body.chat_id || req.body.item_id) || !req.body.content) { Logger.shared.log(`Parameters messing for sending message for chat_id: ${req.body.chat_id} concering item: ${req.body.item_id}`, 1); throw "Missing parameters"; }

		const user = await User.findOne({ uid: req.body.uid });
		const userId = user._id;
		if (!userId) { Logger.shared.log(`Authenticating user sending message for chat_id: ${req.body.chat_id} concering item: ${req.body.item_id} failed`, 1); throw "User uid not found"; }
		...
		if (req.body.chat_id) {// I would put that into a put("/updateChat/:id") route, but not important
			...
			const existingChat = await Chat.findOne({ $and: [{ 'item': req.body.item_id }, { $or: [{ 'borrower': userId }, { 'borrower': req.body.recipient }] }] }).populate([{path: 'item', model: "Products", select: ['name']}, {path: 'borrower', model: "Users", select: ['name']}, {path: 'lender', model: "Users", select: ['name']}, {path:'messages.sender', model:'Users', select: ['name']}]);
			if (existingChat) {
